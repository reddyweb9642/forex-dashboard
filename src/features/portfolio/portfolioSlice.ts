...
const slice = createSlice({
  ...
  reducers: {
    trade: {
      reducer: (state, action: PayloadAction<TradePayload & { id: string; time: number }>) => {
        ...
        if (side === 'BUY') {
          ...
          const newAvg = newQty === 0 ? 0 : (pos.avgPrice * pos.quantity + value) / newQty
          pos.quantity = newQty
          pos.avgPrice = newAvg
          state.cash -= value
        } else {
          ...
          // keep avg price for remaining qty
          state.cash += value
        }
...
// derived
export const selectEquity = (s: RootState, prices: Partial<Record<SymbolCode, number>>) => {
  const posVal = (Object.values(s.portfolio.positions) as {symbol: SymbolCode, quantity: number, avgPrice: number}[])
    .reduce((acc, p) => acc + (prices[p.symbol] ?? 0) * p.quantity, 0)
  return s.portfolio.cash + posVal
}

export const selectPnL = (s: RootState, prices: Partial<Record<SymbolCode, number>>) => {
  let pnl = 0
